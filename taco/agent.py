...
from taco.prompt import CoTAPrompt, DirectAnswerPrompt
...
class UserAgent(UserProxyAgent):
    ...
    def initiate_chat(self, assistant, message, task, image_detail='auto'):
        self.step_id = 0
        self.task = task
        self.current_image_id = len(task['images']) # current image id is the number of input images
        self.called_tools = []
        self.new_image_paths = []
    ...
    def receive(
        ...
        if parsed_status:
            if isinstance(self.parser.prompt_generator, DirectAnswerPrompt):
                ...
                if executed_status and 'images' in executed_results:
                    self.new_image_paths += executed_results['images']
