...
class ChatDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if self.data_path.exists() and self.dataset_type != "huggingface":
            ...
            self.image_dir = self.data_path.parent if image_dir is None else Path(image_dir)
            if shuffle:
                random.seed(42)
    ...
    def preprocess(self):
        ...
        for i, item in tqdm(
            ...
            #     print(e)
            #     print(f"Error at {i} with id: {item['id']}")
            #     print(item)
...
def load_data_from_config(data_args, processor):
    ...
    for sub_dataset_config in data_config['data']:
        ...
        image_dir = sub_dataset_config.get('image_dir', None)
        ...
        if sub_dataset_config['format'] == 'chat':
            sub_dataset = ChatDataset(processor, data_path, dataset_type, name, split, max_seq_len, data_args.conv_format,
                data_args.is_master_worker, max_size, shuffle, max_num_images, vl_only, offline_sha=offline_sha, revision=revision, image_dir=image_dir)
