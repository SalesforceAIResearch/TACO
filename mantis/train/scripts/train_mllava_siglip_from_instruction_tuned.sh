...
if [ "$TRANSFORMERS_OFFLINE" = 1 ]; then
    ...
    model_name_or_path="TIGER-Lab/Mantis-8B-siglip-llama3"
fi
...
if [ $lora_enabled = true ]; then
    ...
    if [ $qlora_enabled = true ]; then
        echo "qlora & dora is enabled"
         RUN_NAME="${RUN_NAME}-qlora_${lora_enabled}-seq_length_${max_seq_len}-lr_${learning_rate}-global_bs_${global_batch_size}-epoch_${max_epochs}-qlora"
    else
         RUN_NAME="${RUN_NAME}-lora_${lora_enabled}-seq_length_${max_seq_len}-lr_${learning_rate}-global_bs_${global_batch_size}-epoch_${max_epochs}-lora"
    fi
else
    echo "lora is disabled"
    RUN_NAME="${RUN_NAME}-seq_len_${max_seq_len}-lr_${learning_rate}-ep_${max_epochs}"
fi
